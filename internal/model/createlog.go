...
type Log struct {
	...
	Version int          `json:"version" form:"version"` // 版本号(代表当前收到的content切片数量)
...
func CreateLog(log *Log, logcontents []LogContent) {
	...
	if logcon.Id != 0 { // 若是存在 则执行覆盖操作
		...
		if len(logcontents) >= count { //若是更新的数量大于数据库中的记录数，正常更新
		...
		} else { // 若是更新的数量小于数据库中的数据数量，删除此tab的更新记录重新插入（保证数据库中没有垃圾数据）
			db.Delete(LogContent{}, "log_id = ?", id)
